			...
			if (is_single() || is_page() || (function_exists('is_shop') && is_shop()) || is_archive() || is_search() || is_404() || is_home()) {
				if (!is_page_template('template-homepage.php')) {
					...
					if (function_exists('is_shop') && (is_shop() || function_exists('is_product') && is_product())) {
						$prime_education_default_image_url = get_template_directory_uri() . '/images/default-header.png'; 
						echo '<img src="' . esc_url($prime_education_default_image_url) . '" alt="Default Image" itemprop="image">';
					} else {
						if (has_post_thumbnail()) {
							(is_active_sidebar('right-sidebar')) ? the_post_thumbnail('prime-education-with-sidebar', array('itemprop' => 'image')) : the_post_thumbnail('prime-education-without-sidebar', array('itemprop' => 'image'));
						} else {
							$prime_education_default_image_url = get_template_directory_uri() . '/images/default-header.png'; 
							echo '<img src="' . esc_url($prime_education_default_image_url) . '" alt="Default Image" itemprop="image">';
						}
					...
					prime_education_custom_blog_banner_title();
					echo '</div>';
				}
			}
		}
