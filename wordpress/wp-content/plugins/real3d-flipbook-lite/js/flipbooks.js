...
(function ($) {
  ...
  if (typeof flipbooks_json != "undefined") {
    downloadObjectAsJson(flipbooks_json[0], "flipbooks");
  }
  ...
  R3D.EditFlipbooks = function () {
    this.books = $.parseJSON(flipbooks[0]);
    var arr = [];
    ...
    function addBook(book) {
      $(
        "<tr>" +
          ...
          book.id +
          '"><div style="height:100px; width: 70px;background-image:url(' +
          (book.thumb || emptyImgSrc) +
          ');background-color:transparent;background-size:contain;background-position:center;background-repeat:no-repeat;">' +
          "</a>" +
          ...
          book.id +
          '"><a href="#" title="Duplicate flipbook" >Duplicate</a> | </span><span class="trash" name="' +
          book.id +
          '" ><a href="#" title="Move to trash" >Trash</a></span>' +
          "</div>" +
          "</td>" +
          '<td>[real3dflipbook id="' +
          book.id +
          '"]   <div id="' +
          book.id +
    ...
    for (var key in this.books) {
      keys.push(key);
      if (typeof this.books[key].date == "undefined") this.books[key].date = "";
    }
    ...
    $("#name").click(function () {
      ...
      if ($(this).hasClass("asc")) self.books.reverse();
      showPage(0);
    });
    ...
    function showPage(index) {
      ...
      $(".edit").click(function (e) {
        ...
        window.location =
          ...
          "?page=real3d_flipbook_admin&action=edit&bookId=" +
          id;
      });
      ...
      $(".undo").click(function (e) {
        ...
        window.location =
          window.location.origin +
          window.location.pathname +
          "?page=real3d_flipbook_admin&action=undo";
      });
    }
    ...
    function duplicateFlipbook(id) {
      ...
      $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
          location.href =
            location.origin + location.pathname + "?page=real3d_flipbook_admin";
        },
    ...
    function deleteFlipbooks(arr) {
      var msg = "";
      var data = "action=r3d_delete&security=" + window.r3d_nonce[0];

      if (arr) {
        if (arr.length == 1) msg = "Deleete flipbook " + arr;
        else msg = "Delete flipbooks " + arr;
        data += "&currentId=" + arr;
      } else {
